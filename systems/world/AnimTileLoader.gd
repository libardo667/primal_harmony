...
func setup_map_animations(map_node: Node) -> void:
	...
	var ts: TileSet = terrain.tile_set
	if ts == null:
		return
	...
	for static_src_id in range(8):
		...
		var is_bottom: bool = stem.ends_with("_bottom")
		...
		var side_key := "bottom_src" if is_bottom else "top_src"
		var anim_src: TileSetAtlasSource = entry.get(side_key)
		if anim_src == null:
			continue

		# Add anim source to TileSet if not already there
		var anim_src_id := _find_source_id_for(ts, anim_src)
		if anim_src_id == -1:
			anim_src_id = ts.add_source(anim_src)
	...
	# Re-route cells on terrain layer 0 (all bottom sub-tiles) and layer 1 (COVERED top
	# sub-tiles, added by batch_paint_maps.gd for water/path/ground metatiles), then
	# decoration layer 0 (NORMAL/SPLIT top sub-tiles that render above the player).
...
func _load_tileset_anim(ts_name: String) -> Dictionary:
	...
	if bottom_src == null and top_src == null:
...
func _find_source_id_for(ts: TileSet, anim_src: TileSetAtlasSource) -> int:
	for i in range(16):  # Check up to 16 source slots
		if ts.has_source(i) and ts.get_source(i) == anim_src:
			return i
	return -1
